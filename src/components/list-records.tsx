...
interface ListRecordsProps extends Record<string, unknown> {
  baseId?: string;
  tableId?: string;
  records?: any[];
  offset?: string;
...
const ListRecords: React.FC<ListRecordsProps> = () => {
  const props = useWidgetProps<ListRecordsProps>({
    baseId: '',
    tableId: '',
    records: [],
    offset: undefined,
  ...
  const handleViewRecord = async (recordId: string) => {
    if (typeof window !== 'undefined' && window.openai?.callTool) {
      try {
        await window.openai.callTool('get_record', {
          baseId,
          tableId,
          recordId,
        });
  ...
  const renderCellValue = (value: any, fieldName: string) => {
    // Handle arrays (choice fields, linked records)
    if (Array.isArray(value)) {
      if (value.length === 0) return <span className="text-gray-400 dark:text-gray-500">—</span>;
      
      // Check if it's a choice/select field (strings) or linked records (objects)
            ...
            {records.map((record: any, recordIdx: number) => (
              <div key={record.id || recordIdx}>
                <button
                  ...
                  className={`w-full flex items-center hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 ${
                    selectedRecord === record.id ? 'bg-gray-50 dark:bg-gray-800/50' : ''
                  }`}
                  style={{ height: '38px' }}
                >
                  {displayFields.map((fieldName, idx) => {
