...
interface GetRecordProps extends Record<string, unknown> {
  baseId?: string;
  tableId?: string;
  record?: any;
}
...
const GetRecord: React.FC<GetRecordProps> = () => {
  const props = useWidgetProps<GetRecordProps>({
    baseId: '',
    tableId: '',
    record: null,
  });

  const { baseId, tableId, record } = props;
  const [selectedRecord] = useState<string | null>(record?.id || null);

  const handleViewRecord = async () => {
    if (typeof window !== 'undefined' && window.openai?.callTool && baseId && tableId && record?.id) {
      try {
