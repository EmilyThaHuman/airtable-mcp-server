...
interface CreateRecordsProps extends Record<string, unknown> {
  baseId?: string;
  tableId?: string;
...
const CreateRecords: React.FC<CreateRecordsProps> = () => {
  const props = useWidgetProps<CreateRecordsProps>({
    baseId: '',
    tableId: '',
    fields: {},
    createdRecord: null,
  ...
  const { baseId, tableId, fields = {}, createdRecord } = props;
  const [editingCell, setEditingCell] = useState<string | null>(null);
  const [editValue, setEditValue] = useState<string>('');
  const [localRecord, setLocalRecord] = useState<any>(null);

  // Initialize or update local record
  useEffect(() => {
    if (createdRecord) {
      setLocalRecord(createdRecord);
    } else if (fields && Object.keys(fields).length > 0) {
      // Create a mock record from fields for display
      setLocalRecord({
        id: 'new',
        fields: fields,
      });
  ...
  const handleSave = async (fieldName: string) => {
    ...
    // If this is a new record (not yet created), just update local state
    // The actual creation will happen when the AI calls create_record
    if (localRecord.id === 'new') {
      // Optionally trigger create_record here if you want immediate creation
      try {
        if (typeof window !== 'undefined' && window.openai?.callTool && baseId && tableId) {
          await window.openai.callTool('create_record', {
            baseId,
            tableId,
    ...
    return (
      <div className="w-full h-full flex items-center justify-center p-8 bg-white dark:bg-gray-900">
        <div className="text-center">
          ...
          <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100 mb-1">No record data</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400">
            No fields provided for record creation.
          </p>
        </div>
      </div>
    );
  ...
  const recordFields = localRecord.fields || {};
      ...
      <div className="border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden bg-white dark:bg-gray-800 flex flex-col h-full">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-4 py-3">
          <div className="flex flex-col items-start">
            <span className="text-lg font-semibold text-gray-900 dark:text-gray-100">
              {createdRecord ? 'Created Record' : 'New Record'}
            </span>
        ...
        <div className="flex items-center justify-between border-t border-gray-200 dark:border-gray-700 px-4 py-2 text-xs text-gray-600 dark:text-gray-400 font-medium bg-white dark:bg-gray-800">
          <span className="flex items-center justify-between w-full">
            {createdRecord ? 'Record created successfully' : 'Ready to create record'}
            <button
