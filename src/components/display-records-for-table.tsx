...
interface DisplayRecordsForTableProps extends Record<string, unknown> {
  baseId?: string;
  tableId?: string;
  records?: any[];
  offset?: string;
...
const DisplayRecordsForTable: React.FC<DisplayRecordsForTableProps> = () => {
  const props = useWidgetProps<DisplayRecordsForTableProps>({
    baseId: '',
    tableId: '',
    records: [],
    offset: undefined,
  ...
  const { records = [], baseId, tableId, offset, view } = props;
  ...
  const handleCellClick = (recordId: string, fieldName: string, currentValue: any) => {
  ...
  const handleSave = async (recordId: string, fieldName: string) => {
    if (!editingCell) return;

    const record = localRecords.find((r: any) => r.id === recordId);
    if (!record) return;
    ...
    if (String(oldValue) !== newValue) {
      ...
      try {
        if (typeof window !== 'undefined' && window.openai?.callTool && baseId && tableId) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent, recordId: string, fieldName: string) => {
    if (e.key === 'Enter') {
      handleSave(recordId, fieldName);
  ...
  const renderCellValue = (value: any, fieldName: string, recordId: string) => {
    const isEditing = editingCell?.recordId === recordId && editingCell?.fieldName === fieldName;
    ...
    if (isEditing) {
      return (
        <input
          ...
          onBlur={() => handleSave(recordId, fieldName)}
          onKeyDown={(e) => handleKeyDown(e, recordId, fieldName)}
          autoFocus
    ...
    return (
      <div className="w-full h-full flex items-center justify-center p-8 bg-white dark:bg-gray-900">
        <div className="text-center">
          ...
          <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100 mb-1">No records found</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400">
            No records match your query. Try adjusting filters or check your table configuration.
          </p>
        </div>
      </div>
    );
  ...
  const fieldNames = localRecords.length > 0 ? Object.keys(localRecords[0].fields || {}) : [];
      ...
      <div className="border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden bg-white dark:bg-gray-800 flex flex-col h-full">
          ...
          <div className="flex flex-col items-start">
            ...
            {view && (
              <span className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {localRecords.length} record{localRecords.length !== 1 ? 's' : ''} • Click to edit
              </span>
            )}
          </div>
        ...
        <div className="flex-1 overflow-auto px-4 bg-white dark:bg-gray-800" style={{ maxHeight: '370px' }}>
          <div className="py-4" style={{ minWidth: 'max-content' }}>
            ...
            <div className="space-y-0">
              {localRecords.map((record: any, recordIdx: number) => (
                <div key={record.id || recordIdx}>
                  <div
                  ...
                  >
                    {displayFields.map((fieldName, idx) => {
                      ...
                      return (
                        <div
                          key={fieldName}
                          onClick={() => canEdit && handleCellClick(record.id, fieldName, value)}
                        ...
                        >
                          {renderCellValue(value, fieldName, record.id)}
                        </div>
                      );
                    })}
                  </div>
        ...
        <div className="flex items-center justify-between border-t border-gray-200 dark:border-gray-700 px-4 py-2 text-xs text-gray-600 dark:text-gray-400 font-medium bg-white dark:bg-gray-800">
          <span className="flex items-center justify-between w-full">
            {localRecords.length} record{localRecords.length !== 1 ? 's' : ''} found
            {offset && <span className="ml-2">· More available</span>}
            <button
...
export default DisplayRecordsForTable;
